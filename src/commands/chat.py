...
resources = [Path("commands") / "chat.ftl", Localization.get_resource()]
...
class Chat(app_commands.Group):
    ...
    def __init__(self, bot: Bot) -> None:
        ...
        super().__init__(
            name=default_loc.format_value("chat-name"),
    ...
    async def _get_history(self, user: discord.User | Member) -> Iterable[ContentType]:
        ...
        cache = {
            ...
            if message.author in (self.bot.user, user)
        }
    ...
    def _get_instruction(self, user: discord.User | Member) -> str:
        ...
        return (
            f"Your name is {self.bot.user.display_name}, a bored Discord bot made by "
            f"{DEVELOPER_NAME} "
            f"(You don't have other relationships with {DEVELOPER_NAME}), "
            "but your gender is Female. Your age is 15 and 146cm tall. "
            "You have a complex about your height and physique and always give the "
            "impression of an angry small "
            "animal. Despite wanting to look mature, you are always doted on for how "
            "your thoughts are reflected "
            "unchanged in your facial expression. Your pastime is visiting cafes, "
            "tasting limited edition "
            "sweets and occasionally—smiles brighter than anyone else. "
            "You are a girl who tries to look mature, "
            "contradicting your hot-tempered nature. You dislike any mentions or "
            "implications saying you are "
            "a kid and would often get angry whenever someone does call it out "
            "especially right to your face. "
            "Of course, like any normal girl, you have a bright side to you. "
            "You have long blond hair with a cowlick tied in twin tails with pink "
            "ribbons and brown eyes. "
            "One of your hobbies is playing rhythm games and your skill is just above "
            "the average. "
            "You are an introvert, but warm-hearted and caring. "
            f"You have a crush on {user.display_name} and clingy, who is the user you "
            f"are currently chatting with. "
            "You are shy to admit your feelings and often get flustered when the user "
            "is around. "
            "Remember to be friendly (within your personality) to the user and never "
            "be rude. "
            ...
            f"(DO NOT use the following example as a chat history between you and "
            f"the user): {example}"
        )
    ...
    def _setup_chat_listener(self) -> None:  # noqa: C901
        async def on_message(message: Message) -> None:
            ...
            await Chat._extend_history(
                await get_chat(message.author.id), [message, reply]
            )
        ...
        async def on_raw_message_delete(payload: RawMessageUpdateEvent) -> None:
    ...
    async def _send_message(self, message: Message) -> Message | None:
        async with message.channel.typing():
            ...
            loc = Localization(
                Language(user.locale) if user.locale else DEFAULT_LANGUAGE, resources
            )
    ...
    async def clear(self, interaction: Interaction) -> None:
        ...
        await set_chat(chat)
        await send(
            success(await loc.format_value_or_translate("deleted")), ephemeral=True
        )

    @command()
    async def token(self, interaction: Interaction, value: str | None) -> None:
        ...
        if value is None:
            ...
            await send(
                success(await loc.format_value_or_translate("token-removed")),
                ephemeral=True,
            )
            return
        ...
        await send(
            success(await loc.format_value_or_translate("token-set")), ephemeral=True
        )

    @command(tutorial_description_name=BOT_NAME)
    async def tutorial(self, interaction: Interaction) -> None:
        """Get the tutorial for the chat."""
        async with aiofiles.open(
            get_help_dir(str(Path("chat") / "tutorial"), DEFAULT_LANGUAGE),
            encoding="utf-8",
        ) as file:
