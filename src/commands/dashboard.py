"""Contains the dashboard command."""
...
async def dashboard(interaction: Interaction) -> None:
    ...
    languages = [
        f"`{await loc.format_value_or_translate(code)}`" for code in user.translate_to
    ]
    languages.sort()
    ...
    user_embed = (
        ...
        .add_field(
            name=await loc.format_value_or_translate(
                "history-length", {"name": BOT_NAME}
            ),
    ...
    async def callback(select_interaction: Interaction) -> None:
        ...
        for channel in channel_select.values:
            ...
            languages = [
                f"`{await loc.format_value_or_translate(code)}`"
                for code in config.translate_to
            ]
            languages.sort()

            main_language = (
                await loc.format_value_or_translate("none")
                if config.locale is None
                else f"`{await loc.format_value_or_translate(config.locale)}`"
            )

            embed = (
                Embed(color=color, description=f"<#{channel.id}>")
                .add_field(
                    name=await loc.format_value_or_translate("translation-languages"),
                    value=", ".join(languages)
                    or await loc.format_value_or_translate("none"),
                    inline=False,
                )
                .add_field(
