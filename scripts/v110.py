...
async def replace_translate_to(collection: AsyncIOMotorCollection) -> None:
    """Replace the translate_to field with language code."""
    pbar = tqdm(
        desc=f"Updating {collection.name} collection",
    ...
    async for doc in collection.find():
        pbar.update(1)
        doc["translate_to"] = [
            NAME_TO_CODE.get(lang, lang)
            for lang in doc["translate_to"]
